...
export default function CadWorkspace({
  ...
  // 自动把选中产生的 CAD 引用 postMessage 给父页面(供 text2cad 前端自动填充「3D 引用」)。
  useEffect(() => {
    if (!canonicalCopySelectionLines.length) {
      return;
    }
    try {
      window.parent.postMessage(
        { type: "cad-selection", ref: canonicalCopySelectionLines.join(","), refs: canonicalCopySelectionLines },
        "*"
      );
    } catch (_err) {
      // 忽略跨域/父页面未监听等错误
