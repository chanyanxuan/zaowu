...
const PROJECTION_MODE_OPTIONS = [
  { value: CAMERA_PROJECTION.ORTHOGRAPHIC, label: "正交", title: "适合 CAD 检查的平行投影", Icon: OrthographicProjectionIcon },
  { value: CAMERA_PROJECTION.PERSPECTIVE, label: "透视", title: "带透视消失线的深度投影", Icon: PerspectiveProjectionIcon }
];
    ...
    <div className="py-2" data-cad-theme-settings-section="true">
      ...
      <ControlSubsection title="表面">
        {themeSettings.materials.cycleColors === true ? (
          <Field label="颜色方案" value={`${resolveFillColors(themeSettings.materials).length}/${MAX_THEME_FILL_COLORS}`}>
            <FillColorEditor
        ...
        <SliderField label="清漆层" value={formatNumber(themeSettings.materials.clearcoat)}>
          <SliderInput
      ...
      >
        {themeSettings.floor?.enabled === true ? (
          <>
            ...
            <SliderField label="背景融合" value={formatNumber(themeSettings.floor?.horizonBlend ?? 0)}>
              <SliderInput
        ...
        {themeSettings.floor?.grid?.enabled === true ? (
          <>
            ...
            <ColorModeField
              ...
              {...themeColorFieldProps}
            />
            <SliderField label="线条透明度" value={formatNumber(themeSettings.floor?.grid?.opacity ?? 0.18)}>
              <SliderInput
