...
function stepTreeRowAriaLabel(row, topologyType, detail = "") {
  ...
  const nodeType = String(row?.nodeType || row?.node?.nodeType || "").trim();
  const prefix = nodeType === "assembly" ? "装配体" : "零件";
...
function stepTreeRowTooltip(row, {
  ...
  return [
    formatTreeTooltipLine(type || "条目", label),
    formatTreeTooltipLine("引用", formatRefForTooltip(selector)),
    formatTreeTooltipLine("信息", detail),
    formatTreeTooltipLine("状态", disabledReason),
  ].filter(Boolean).join("\n");
}

function mateRowTooltip(mate, disabledReason = "") {
  return [
    formatTreeTooltipLine("配合", mate?.label),
    formatTreeTooltipLine("引用", formatRefForTooltip(mate?.id)),
    formatTreeTooltipLine("信息", mate?.detail),
    formatTreeTooltipLine("状态", disabledReason),
  ].filter(Boolean).join("\n");
}
...
export default function StepFileSheet({
  ...
  const sections = [
                              ...
                              <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-hidden">
                                {rowHasChildren ? (
                                  <Button
                                    ...
                                    aria-label={rowExpanded ? `折叠 ${row.label}` : `展开 ${row.label}`}
                                    title={rowExpanded ? "折叠" : "展开"}
                                  >
                                    <ChevronRight
    ...
    buildParameterControlsTab({
      ...
      enableAriaLabel: "启用 STEP 模块",
      animationAriaLabel: "STEP 动画",
      resetTitle: "重置 STEP 参数",
