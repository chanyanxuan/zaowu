      ...
      >
        {menu.global === true ? (
          ...
          <AssemblyContextMenuItems
            ...
            hideAllLabel={String(menu.hideAllLabel || "").trim() || "显示全部"}
...
export default function CadRenderPane({
  ...
  const ctaLabel = ctaMode === "screenshot" ? "复制截图" : copyButtonLabel;
  const ctaTitle = ctaMode === "screenshot" ? "复制截图到剪贴板" : copyButtonLabel;
        ...
        >
          ...
          >
            <p className="col-start-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-destructive">
              {missingFileOutsideRoot ? "在该查看器根目录之外" : "文件不存在"}
            </p>
          ...
          >
            ...
            <div className="min-w-0 max-w-full">
              ...
              <div className="mt-1 line-clamp-2 min-w-0 max-w-full break-words text-sm font-medium leading-5 text-foreground">
                {viewerAlert.title || viewerAlert.summary || "查看器问题"}
              </div>
              {viewerAlert.message ? (
